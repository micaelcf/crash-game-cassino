...
import { type RabbitHandle, startRabbit, stopContainer } from '../setup'
...
describe.skipIf(skipIntegration)('rabbit topology', () => {
	...
	it('routes a message published with bet.placed to the wallets queue', async () => {
		...
		channel.publish(
			...
			'bet.placed',
			Buffer.from(
				JSON.stringify({ userId: 'p1', betAmount: '100', roundId: 'r1' }),
			),
			{ contentType: 'application/json', persistent: true },
		)

		const received = await new Promise<amqp.ConsumeMessage>(
			(resolve, reject) => {
