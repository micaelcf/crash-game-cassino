...
[`@crash/contracts`](packages/contracts/src/ws) — the slim package owns
WebSocket payloads + status enums; HTTP DTOs are generated by orval into
[`@crash/api-client`](packages/api-client) from each service's
`openapi.json`) is:
...
flowchart TB
  ...
  subgraph Shared["packages/"]
    direction TB
    SH_Contracts["contracts (@crash/contracts)<br/>WS event types · pagination · status enums"]
    SH_ApiClient["api-client (@crash/api-client)<br/>orval-generated TanStack Query hooks + DTOs<br/>(games + wallets clients, fetch mutator)"]
  end
  ...
  FE -. "WS types" .-> SH_Contracts
  FE -. "HTTP hooks + DTOs" .-> SH_ApiClient
  G_App -. "WS + status types" .-> SH_Contracts
  W_App -. "WS + status types" .-> SH_Contracts
  G_Pres -. "openapi.json" .-> SH_ApiClient
  W_Pres -. "openapi.json" .-> SH_ApiClient
```
...
├── packages/
│   ├── contracts/                # Slim shared package: WS event types,
│   │                             # pagination, bet/round status enums
│   │                             # (@crash/contracts)
│   └── api-client/               # Orval-generated TanStack Query hooks +
│                                 # DTOs for both backends; ./games and
│                                 # ./wallets sub-entries, custom fetch
│                                 # mutator handles Logto bearer + ApiError
│                                 # (@crash/api-client)
...
### Regenerate the API client (`@crash/api-client`)

The frontend consumes TanStack Query hooks generated by **orval** from each
service's `openapi.json`. After changing a controller or DTO, refresh both
the spec and the client in one command:

```bash
bun run swagger:dump     # boots each service headlessly, writes
                         # services/{games,wallets}/openapi.json
bun run codegen          # swagger:dump → orval generation under
                         # packages/api-client/src/{games,wallets}/
```

The generated tree under `packages/api-client/src/{games,wallets}/` is
committed so the contract is reviewable in PRs. CI should fail the build
if `bun run codegen && git diff --exit-code` would produce changes —
catches "controller changed but client wasn't regenerated" drift.

`swagger:dump` needs Postgres + RabbitMQ up (`bun run docker:up:infra` or
just `bun scripts/compose.ts up -d postgres rabbitmq`) since the NestJS
modules connect on init.
...
### 📦 Backend-owned wire contract + Orval-generated client

The backends are the source of truth for HTTP shapes. Controllers carry
decorated NestJS DTOs (`@ApiProperty`, `@ApiOkResponse({ type })`, named
`PagedDto<T>` for paginated responses), each service ships an
`openapi.json` artifact via `bun run swagger:dump`, and
[**orval**](https://orval.dev) generates TanStack Query hooks + types
into `@crash/api-client` (`./games` and `./wallets` entries).

- **Why**: kills the previous "shared TS interface that both sides
  import" pattern — Swagger now has a real schema, the frontend gets
  hook-per-endpoint with proper React Query keys, and adding a field is
  a single backend change followed by `bun run codegen`.
- **Trade-off**: extra build step in the chain. Mitigated by committing
  the generated tree so PR reviewers see exactly what changed and CI
  guards against drift.
- **WS events stay hand-written** in `@crash/contracts` since OpenAPI
  doesn't describe Socket.IO payloads.
...
- ✅ **OpenAPI** spec on both services, rendered with **Scalar** (interactive
  API reference, try-it-out client, dark-mode `saturn` theme) and consumed
  by the frontend through an **orval-generated TanStack Query client**
  (`@crash/api-client`) — adding an endpoint is one backend change +
  `bun run codegen`.
